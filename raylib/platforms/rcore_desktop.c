/**********************************************************************************************
*
*   rcore_desktop - Functions to manage window, graphics device and inputs (modified by M374LX)
*
...
// Note: modified by M374LX to make it similar to ToggleFullscreen() but without a video mode change
void ToggleBorderlessWindowed(void)
{
    if (!CORE.Window.fullscreen)
    {
        ...
        {
            int width = GetMonitorWidth(monitorIndex);
            int height = GetMonitorHeight(monitorIndex);
            int refreshRate = GetMonitorRefreshRate(monitorIndex);
            ...
            //According to GLFW's documentation, the video mode does not change if this function is called with
            //the monitor's current width, height, and refresh rate
